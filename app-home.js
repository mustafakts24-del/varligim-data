...
//
// DÜZELTME (2026-09, "geç gelen bazı kısımlar" bildirimi): yukarıdaki
// düzeltme yalnızca sayfa değişimlerini kapsıyordu — TEK BİR kartın
// (özellikle BTC/USD, ETH/USD; bunlar CoinGecko'ya doğrudan tarayıcıdan
// bağlanıyor ve ücretsiz katmanın sık 429/gecikme sorunu var) o anki
// yenilemesi başarısız olursa, catch bloğu daha önce gösterilen DOĞRU
// değerin üzerine sessizce "—" yazıyordu (mobildeki net worth kripto
// önbellek düzeltmesiyle birebir aynı sınıf hata). Artık her ticker'ın
// son başarılı değeri `_tickerLastKnownGood` içinde saklanıyor; bir
// yenileme başarısız olursa (bir kısa yeniden deneme de dahil) ekranda
// halihazırda gösterilen son bilinen doğru değer KORUNUYOR, sessizce
// "—"ya düşülmüyor. Hiç başarılı veri gelmemişse (ilk yüklemede de
// hata varsa) dürüstçe "—" gösterilmeye devam eder — uydurma veri yok.
const _tickerLastKnownGood = {};
...
async function loadExtraTickerGrid(containerId, tickers) {
  ...
  if (grid.dataset.built !== '1') {
    grid.innerHTML = tickers.map(t => {
      const cached = _tickerLastKnownGood[t.id];
      return `
      <div class="ticker-box">
        <div class="ticker-name">${escapeHtml(t.name)}</div>
        <div class="ticker-value" id="ticker2-value-${t.id}">${cached ? cached.valueText : '…'}</div>
        <div id="ticker2-chip-${t.id}">${cached ? cached.chipHtml : '<span class="chip neu">…</span>'}</div>
      </div>
    `;
    }).join('');
  ...
  await Promise.all(tickers.map(async t => {
    ...
    try {
      ...
      // Geçici bir hata/gecikme olabilir (ör. CoinGecko 429) — hemen
      // pes etmeden kısa bir bekleme sonrası bir kez daha dene.
      await _sleep(600);
      try { quote = await t.fetcher(); } catch (e2) { quote = null; }
    }
    if (quote != null) {
      const valueText = t.fmt(quote.price);
      const chipHtml = changeChipHtml(quote.changePercent);
      valueEl.textContent = valueText;
      chipEl.innerHTML = chipHtml;
      _tickerLastKnownGood[t.id] = { valueText, chipHtml };
    } else {
      const cached = _tickerLastKnownGood[t.id];
      if (cached) {
        // Son bilinen doğru değeri koru — geçici hatada ekranı "—"ya düşürme.
        valueEl.textContent = cached.valueText;
        chipEl.innerHTML = cached.chipHtml;
      } else {
        valueEl.textContent = '—';
...
async function loadHomePage() {
  ...
  // DÜZELTME (2026-09, "veriler gelmiyor" bildirimi): önceden bu
  // fonksiyonun gövdesinde bir hata oluştuğunda (ağ kopması, geçici
  // Supabase hatası, oturum sorunu vb.) — showPage() bu fonksiyonu
  // `await`lemediği/`catch` etmediği için — hata SESSİZCE yutuluyordu.
  // Sonuç: "TOPLAM NET VARLIK" sonsuza kadar "…" da kalıyor, "Varlık
  // Dağılımı" grafiği hiç çizilmiyor, Piyasalar/Emtialar boş kalıyor ve
  // kullanıcı sayfayı yenilemekten başka HİÇBİR ipucu alamıyordu. Artık
  // tüm gövde try/catch içine alındı: hata konsola yazdırılır VE
  // kullanıcıya "Tekrar Dene" butonlu, dürüst bir hata durumu gösterilir
  // (uydurma bir toplam/rakam gösterilmez).
  try {
  ...
  homePageEverLoaded = true;
    ...
    console.error('Ana sayfa yüklenemedi:', e);
    // Daha önce başarıyla yüklenmişse (ör. arka planda geçici bir
    // yenileme hatası) mevcut ekranı bozmadan bırak — yalnızca İLK
    // yüklemede tamamen boş/asılı kalan bir ekran yerine dürüst bir
    // hata durumu göster.
    if (!homePageEverLoaded) {
      totalEl.textContent = '—';
      if (updatedEl) {
        updatedEl.innerHTML = `<span style="color:var(--negative,#FF525D);">Veriler yüklenemedi — internet bağlantınızı kontrol edip tekrar deneyin.</span>`;
      }
      statGrid.innerHTML = `
        <div class="empty" style="grid-column:1/-1; display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
          <span>Ana sayfa verileri yüklenirken bir sorun oluştu.</span>
          <button type="button" class="btn outline small" id="homeRetryBtn">Tekrar Dene</button>
        </div>`;
      const retryBtn = document.getElementById('homeRetryBtn');
      if (retryBtn) retryBtn.addEventListener('click', () => loadHomePage());
    }
  }
}
